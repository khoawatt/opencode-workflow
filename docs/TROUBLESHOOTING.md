...
| Trang `Oops, an error occurred / Route Error (400 Invalid content type)` | Auth0 login transaction bị stale/hết hạn. Bridge giờ tự xóa auth transaction và restart flow cho cả `login --auto` và `login` thủ công; manual giới hạn 3 lần. Nếu vẫn lỗi: `chatgpt-review logout` rồi `chatgpt-review login` để tạo profile sạch; không F5/back giữa flow |
